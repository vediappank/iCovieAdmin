...
export class WingeditComponent implements OnInit {
	...
	filteredBuilding: MBuildingModel[] = [];
	...
	ngOnInit() {
		...
		this.Wing$.subscribe(res => {
			...
			this.Wing.bedcount=res.bedcount;
			this.Wing.squarefeet=res.squarefeet;
			this.Wing.price=res.price;
		...
		this.auth.GetALLLocation().subscribe((users: MLocationModel[]) => {
			...
			// 	this.unfilteredLocations.push(_Location);
			// 	// }
			// });
			...
			// 		remove(this.unfilteredLocations, Location);
			// 	}
			// });
		});
		...
		this.auth.GetALLBuilding().subscribe((users: MBuildingModel[]) => {
			debugger;
			this.allBuildings=users;
			if (this.allBuildings.length > 0) {
				this.filteredBuilding = this.allBuildings.filter(row => row.companyid == Number(this.CompanyIdForAdding));
			}
			// each(users, (_Building: MBuildingModel) => {
			// 	// if (_Building.id == this.BuildingID) {

			// 	this.allBuildings.push(_Building);
			// 	this.unfilteredBuilding.push(_Building);
			// 	// }
			// });
			...
			// 	});
			// 	if (Building) {
			// 		this.filteredBuilding.push(Building);
			// 		remove(this.unfilteredBuilding, Building);
			// 	}
			// });
		});
		...
		this.auth.GetALLFloor().subscribe((users: MFloorModel[]) => {
			this.allFloors=users;
			if (this.allFloors.length > 0) {
				this.filteredFloors = this.allFloors.filter(row => row.companyid == Number(this.CompanyIdForAdding));
			}
			// each(users, (_Floor: MFloorModel) => {
			// 	// if (_Floor.id == this.FloorID) {

			// 	this.allFloors.push(_Floor);
			// 	this.unfilteredFloors.push(_Floor);
			// 	// }
			// });
			...
			// 	});
			// 	if (Floor) {
			// 		this.filteredFloors.push(Floor);
			// 		remove(this.unfilteredFloors, Floor);
			// 	}
			// });
		});
	}
	...
	prepareWing(): MWingModel {
		...
		_Wing.bedcount = this.Wing.bedcount;
		_Wing.squarefeet = this.Wing.squarefeet;
		_Wing.price = this.Wing.price;
	...
	getLocationByCompany() {
		...
		this.getBuildingByLocation();
	}

	getBuildingByLocation() {
		if (this.allBuildings.length > 0) {
			this.filteredBuilding = this.allBuildings.filter(row => row.locationid == Number(this.LocationIdForAdding));
		}
		this.BuildingIdForAdding = this.filteredBuilding[0].id;
