...
{
    public class covieadminController : ApiController
    {
        ...
        public HttpResponseMessage DeleteCompany(int CompanyID)
        {
            ...
            try
            {
                ...
                using (SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["iCovCon"].ToString()))
                {
                    ...
                    SqlCommand cmd = new SqlCommand("SP_MCOMPANY_DELETE", con);
                    cmd.CommandType = CommandType.StoredProcedure;
        ...
        public HttpResponseMessage DeleteRegion(int RegionID)
        {
            ...
            try
            {
                ...
                using (SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["iCovCon"].ToString()))
                {
                    ...
                    SqlCommand cmd = new SqlCommand("SP_MREGION_DELETE", con);
                    cmd.CommandType = CommandType.StoredProcedure;
                    ...
                    if (dtData.Rows.Count > 0)
                    {
                        ...
                        for (int i = 0; i < dtData.Rows.Count; i++)
                        {
                            ...
                            mItem = new StateModel
                            {
                                companyid = StringUtil.getInt(vals[0].ToString()),
                                companyname = StringUtil.nullIfEmpty(vals[1].ToString()),
                                countryid = StringUtil.getInt(vals[2].ToString()),
                                countryname = StringUtil.nullIfEmpty(vals[3].ToString()),
                                id = StringUtil.getInt(vals[4].ToString()),
                                statename = StringUtil.nullIfEmpty(vals[5].ToString()),
        ...
        public HttpResponseMessage InsertState(StateModel State)
        {
            ...
            try
            {
                ...
                using (SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["iCovCon"].ToString()))
                {
                    ...
                    cmd.Parameters.AddWithValue("@I_COMPANYID", State.companyid);
                        ...
                        for (int i = 0; i < dtData.Rows.Count; i++)
                        {
                            ...
                            mItem = new CityModel
                            {
                                ...
                                shortname = StringUtil.nullIfEmpty(vals[8].ToString()),
                                  companyid = StringUtil.getInt(vals[9].ToString()),
                                companyname = StringUtil.nullIfEmpty(vals[10].ToString()),
                                imagename = StringUtil.nullIfEmpty(vals[11].ToString()),
                                imagepath = StringUtil.nullIfEmpty(vals[12].ToString()),
                                showimagepath = StringUtil.nullIfEmpty(vals[13].ToString())
                            };
                            mItems.Add(mItem);
                        }
                    }
        ...
        public HttpResponseMessage InsertCity(CityModel City)
        {
            ...
            try
            {
                ...
                using (SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["iCovCon"].ToString()))
                {
                    ...
                    cmd.Parameters.AddWithValue("@I_COMPANYID", City.companyid);
        ...
        [HttpPost]
        [ActionName("AddCityImage")]
        public List<CityImagesModel> AddCityImage()
        {
            string ErrorMessage = string.Empty;
            List<CityImagesModel> objListModel = new List<CityImagesModel>();
            try
            {
                string root = ConfigurationManager.AppSettings["ImgUploadPath"].ToString();
                ...
                CityImagesModel objModel = new CityImagesModel();

                foreach (string file in httpRequest.Files)
                {
                    objModel = new CityImagesModel();
                    var postedFile = httpRequest.Files[file];
                    var filePath = System.IO.Path.Combine(root + "\\UploadCity\\", postedFile.FileName);
                    postedFile.SaveAs(filePath);
                    objModel.imagepath = root + "\\UploadCity\\" + postedFile.FileName;
                    objModel.imagename = postedFile.FileName;
                    objModel.showimagepath = "./assets/UploadCity/" + postedFile.FileName;
                    objModel.cityid = Convert.ToInt32(file.Split('-')[1]);
        ...
        [HttpPost]
        [ActionName("UpdateCityImages")]
        public HttpResponseMessage UpdatePropertyImages(List<CityImagesModel> CityImages)
        {
            string errorDesc = string.Empty;
            string errorCode = string.Empty;
            try
            {
                ...
                log.DebugFormat("Retrieving UpdateCityImages");
                foreach (CityImagesModel objimages in CityImages)
                {
                    using (SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["iCovCon"].ToString()))
                    {
                        SqlParameter outErrorCode = new SqlParameter("@o_ErrorCode", SqlDbType.Int) { Direction = ParameterDirection.Output };
                        ...
                        SqlCommand cmd = new SqlCommand("SP_MCITY_IMAGES_UPDATE", con);
                        cmd.CommandType = CommandType.StoredProcedure;
                        cmd.Parameters.AddWithValue("@I_CITYID", objimages.cityid);
                        ...
                        errorCode = outErrorCode.Value.ToString();
                        errorDesc = outErrorDesc.Value.ToString();
                        log.DebugFormat("UpdateCityImages Result:{0}", errorDesc);
                        log.DebugFormat("Procedure executed successfully, UpdateCityImages Records returned-{0}", errorDesc);
                        log.Info("Insert Data:::" + JsonConvert.SerializeObject(errorDesc));
                    }
            ...
            {
                log.ErrorFormat("Exception in UpdateCityImages method in database layer:{0}|{1}", ex.Message, ex.StackTrace);
            }
            return Request.CreateResponse(HttpStatusCode.OK, errorDesc, "application/json");
        }
        #endregion
...
//                    cmd.Parameters.AddWithValue("@I_BUILDINGNAME", Building.buildingimage == null ? string.Empty : Building.buildingimage);
        ...
        public HttpResponseMessage InsertFloor(FloorModel Floor)
        {
            ...
            try
            {
                ...
                if (Floor.floorcheckedlist != null && Floor.floorcheckedlist.Count > 0)
                {
                    string flag = "Y";
                    foreach (var ar in Floor.floorcheckedlist)
                    {
                        using (SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["iCovCon"].ToString()))
                        {
                            SqlParameter outErrorCode = new SqlParameter("@o_ErrorCode", SqlDbType.Int) { Direction = ParameterDirection.Output };
                            ...
                            cmd.Parameters.AddWithValue("@I_BuildingID", ar.id);
                            cmd.Parameters.AddWithValue("@I_FloorName", ar.name);
                            cmd.Parameters.AddWithValue("@I_SHORTNAME", ar.name);
                    ...
                    if (dtData.Rows.Count > 0)
                    {
                        ...
                        for (int i = 0; i < dtData.Rows.Count; i++)
                        {
                            ...
                            mItem = new FloorModel
                            {
                                ...
                                shortname = StringUtil.nullIfEmpty(vals[8].ToString()),
                                floorcheckedlist = GetFloorBuilding(dtData)
                            };
                            mItems.Add(mItem);
                        }
                    }
        ...
        public List<FloorBuildingModel> GetFloorBuilding(DataTable dtData)
        {
            List<FloorBuildingModel> list = new List<FloorBuildingModel>();

            FloorBuildingModel objmodel = null;

            try
            {
                log.DebugFormat("Retrieving GetFloorBuilding");
                if (dtData.Rows.Count > 0)
                {
                    for (int i = 0; i < dtData.Rows.Count; i++)
                    {
                        var vals = dtData.Rows[i].ItemArray;
                        objmodel = new FloorBuildingModel
                        {
                            id = StringUtil.getInt(vals[4].ToString()),
                            name = StringUtil.nullIfEmpty(vals[7].ToString()),
                         
                        };
                        list.Add(objmodel);
                    }
                    log.DebugFormat("GetFloorBuilding Inputs :  {0}", dtData);
                }

            }
            catch (Exception ex)
            {
                log.ErrorFormat("Exception in GetFloorBuilding method in database layer:{0}|{1}", ex.Message, ex.StackTrace);
            }
            return list;
        }
        ...
        public HttpResponseMessage DeleteUnit(int UnitID)
        {
            ...
            try
            {
                ...
                using (SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["iCovCon"].ToString()))
                {
                    ...
                    cmd.CommandType = CommandType.StoredProcedure;
                    cmd.Parameters.AddWithValue("@I_ID", UnitID);
                    ...
                    if (dtData.Rows.Count > 0)
                    {
                        ...
                        for (int i = 0; i < dtData.Rows.Count; i++)
                        {
                            ...
                            mItem = new UnitModel
                            {
                                ...
                                bedcount = StringUtil.getInt(vals[13].ToString()),
                                squarefeet = StringUtil.nullIfEmpty(vals[14].ToString()),
                                price = StringUtil.getDecimal(vals[15].ToString()),
                ...
                using (SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["iCovCon"].ToString()))
                {
                    ...
                    if (dtData.Rows.Count > 0)
                    {
                        ...
                        for (int i = 0; i < dtData.Rows.Count; i++)
                        {
                            ...
                            {
                                id = StringUtil.getInt(vals[0].ToString()),
                                companyid = StringUtil.getInt(vals[1].ToString()),
                                companyname = StringUtil.nullIfEmpty(vals[2].ToString()),
                                locationid = StringUtil.getInt(vals[3].ToString()),
                                locationname = StringUtil.nullIfEmpty(vals[4].ToString()),
                                buildingid = StringUtil.getInt(vals[5].ToString()),
                                buildingname = StringUtil.nullIfEmpty(vals[6].ToString()),
                                floorid = StringUtil.getInt(vals[7].ToString()),
                                floorname = StringUtil.nullIfEmpty(vals[8].ToString()),
                                unitsid = StringUtil.getInt(vals[9].ToString()),
                                units = StringUtil.nullIfEmpty(vals[10].ToString()),
                                unitstype = StringUtil.nullIfEmpty(vals[11].ToString())
                               // shortname = StringUtil.nullIfEmpty(vals[4].ToString())
                    ...
                    if (dtData.Rows.Count > 0)
                    {
                        ...
                        for (int i = 0; i < dtData.Rows.Count; i++)
                        {
                            ...
                            mItem = new AminitiesModel
                            {
                                ...
                                icon = StringUtil.nullIfEmpty(vals[2].ToString())
                    ...
                    if (dtData.Rows.Count > 0)
                    {
                        ...
                        for (int i = 0; i < dtData.Rows.Count; i++)
                        {
                            ...
                            mItem = new PropertyModel
                            {
                                ...
                                NeighbourCategoryTypeList = GetAllPropertyNeighbourCategoryTypes(StringUtil.getInt(vals[26].ToString())),
                                ImagesList = GetAllPropertyImages(StringUtil.getInt(vals[26].ToString())),

                                discountpricing = StringUtil.getDecimal(vals[27].ToString()),
                                bedtype = StringUtil.getInt(vals[28].ToString()),
                                bedtypename = StringUtil.nullIfEmpty(vals[29].ToString()),
                                businesscategory = StringUtil.getInt(vals[30].ToString()),
                                businesscategoryname = StringUtil.nullIfEmpty(vals[31].ToString()),
                                guesttype = StringUtil.getInt(vals[32].ToString()),
                                guesttypename = StringUtil.nullIfEmpty(vals[33].ToString()),
                                propertycategory = StringUtil.getInt(vals[34].ToString()),
                                propertycategoryname = StringUtil.nullIfEmpty(vals[35].ToString()),
                                propertytype = StringUtil.getInt(vals[36].ToString()),
                                propertytypename = StringUtil.nullIfEmpty(vals[37].ToString()),
                                propertyname = StringUtil.nullIfEmpty(vals[38].ToString()),
                                yearbuild = StringUtil.nullIfEmpty(vals[39].ToString()),
                                propertyvideo = StringUtil.nullIfEmpty(vals[40].ToString()),
                                gender = StringUtil.getInt(vals[41].ToString()),
                                gendername = StringUtil.nullIfEmpty(vals[42].ToString()),
                            };
                            mItems.Add(mItem);
                        }
                    }
        ...
        public int InsertProperty(PropertyModel Property)
        {
            ...
            try
            {
                ...
                using (SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["iCovCon"].ToString()))
                {
                    ...
                    cmd.Parameters.AddWithValue("@I_discountpricing", Property.discountpricing);
                    cmd.Parameters.AddWithValue("@I_bedtype", Property.bedtype);
                    cmd.Parameters.AddWithValue("@I_businesscategory", Property.businesscategory);
                    cmd.Parameters.AddWithValue("@I_guesttype", Property.guesttype);
                    cmd.Parameters.AddWithValue("@I_propertycategory", Property.propertycategory);
                    cmd.Parameters.AddWithValue("@I_propertytype", Property.propertytype);
                    cmd.Parameters.AddWithValue("@I_propertyname", Property.propertyname);
                    cmd.Parameters.AddWithValue("@I_yearbuild", Property.yearbuild);
                    cmd.Parameters.AddWithValue("@I_propertyvideo", Property.propertyvideo);
                    cmd.Parameters.AddWithValue("@I_gender", Property.gender);
                    ...
                    errorDesc = InsertPropertyNeighbourCategoryType(Property.NeighbourCategoryTypeList, Convert.ToInt32(errorCode));
        ...
        public List<PropertyNeighbourCategorryTypeModel> GetAllPropertyNeighbourCategoryTypes(int propertyid)
        {
            DataTable dtData = null;
            List<PropertyNeighbourCategorryTypeModel> mItems = new List<PropertyNeighbourCategorryTypeModel>();
            PropertyNeighbourCategorryTypeModel mItem = null;
            SqlDataAdapter adapter = null;
            string jsonData = string.Empty;
            try
            {
                ...
                using (SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["iCovCon"].ToString()))
                {
                    SqlParameter outErrorCode = new SqlParameter("@o_ErrorCode", SqlDbType.Int) { Direction = ParameterDirection.Output };
                    SqlParameter outErrorDesc = new SqlParameter("@o_ErrorDescription", SqlDbType.VarChar, 5000) { Direction = ParameterDirection.Output };
                    SqlCommand cmd = new SqlCommand("SP_GET_ALL_MPROPERTY_NEIGHBOUR_CATEGORY_TYPE", con);
                    cmd.CommandType = CommandType.StoredProcedure;
                    cmd.Parameters.AddWithValue("@I_PROPERTYID", propertyid);
                    cmd.Parameters.Add(outErrorCode);
                    cmd.Parameters.Add(outErrorDesc);
                    ...
                    if (dtData.Rows.Count > 0)
                    {
                        mItems = new List<PropertyNeighbourCategorryTypeModel>();
                        for (int i = 0; i < dtData.Rows.Count; i++)
                        {
                            var vals = dtData.Rows[i].ItemArray;
                            mItem = new PropertyNeighbourCategorryTypeModel
                            {
                                // propertyid = StringUtil.getInt(vals[0].ToString()),
                                PropertyNeighbourCategoryid = StringUtil.getInt(vals[1].ToString()),
                                PropertyNeighbourTypeid = StringUtil.getInt(vals[2].ToString())
        ...
        public string InsertPropertyNeighbourCategoryType(List<PropertyNeighbourCategorryTypeModel> propertyneighbourcategorytype, int propertyid)
        {
            string errorDesc = string.Empty;
            string errorCode = string.Empty;
            try
            {
                string Flag = "Y";
                log.DebugFormat("Retrieving Insert PropertyNeighbours");
                foreach (PropertyNeighbourCategorryTypeModel objNeighbours in propertyneighbourcategorytype)
                {
                    using (SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["iCovCon"].ToString()))
                    {
                        SqlParameter outErrorCode = new SqlParameter("@o_ErrorCode", SqlDbType.Int) { Direction = ParameterDirection.Output };
                        ...
                        SqlCommand cmd = new SqlCommand("SP_MPROPERTY_NEIGHBOURMAPPING_INSERT", con);
                        cmd.CommandType = CommandType.StoredProcedure;
                        cmd.Parameters.AddWithValue("@I_PROPERTYID", propertyid);
                        cmd.Parameters.AddWithValue("@I_NEIGHBOURCATEGORY", objNeighbours.PropertyNeighbourCategoryid);
                        cmd.Parameters.AddWithValue("@I_NEIGHBOURTYPE", objNeighbours.PropertyNeighbourTypeid);
                        cmd.Parameters.AddWithValue("@I_FLAG", Flag);
                        cmd.Parameters.Add(outErrorCode);
                        cmd.Parameters.Add(outErrorDesc);
                        con.Open();
                        ...
                        errorCode = outErrorCode.Value.ToString();
                        errorDesc = outErrorDesc.Value.ToString();
                        log.DebugFormat("Insert PropertyNeighbours Result:{0}", errorDesc);
                        log.DebugFormat("Procedure executed successfully, Insert PropertyNeighbours Records returned-{0}", errorDesc);
                        log.Info("Insert Data:::" + JsonConvert.SerializeObject(errorDesc));
                    }
                    Flag = "N";
                }
            ...
            {
                log.ErrorFormat("Exception in InsertPropertyNeighbours method in database layer:{0}|{1}", ex.Message, ex.StackTrace);
            }
            return errorDesc;
        }
        ...
        public HttpResponseMessage RoleMenuMappingUserInsert(RoleModel rmmodel)
        {
            ...
            try
            {
                ...
                foreach (var mappinpItem in rmmodel.permissions)
                {
                    using (SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["iCovCon"].ToString()))
                    {
                        ...
                        if(StringUtil.getInt(errorCode) ==-1)
                        {   
                            break;
                        }
            ...
            //return new CreateResponse(HttpStatusCode.OK,) { Content = new StringContent(errorDesc) };
            return Request.CreateResponse(HttpStatusCode.OK, errorDesc +'~'+ errorCode, "application/json");
        }
        ...
        public HttpResponseMessage DeleteRole(int role_id)
        {
            ...
            try
            {
                ...
                using (SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["iCovCon"].ToString()))
                {
                    ...
                    SqlCommand cmd = new SqlCommand("SP_MROLE_DELETE", con);
                    cmd.CommandType = CommandType.StoredProcedure;
        ...
        [HttpGet]
        [ActionName("GetAllGender")]
        public List<GenderModel> GetAllGender()
        {
            DataTable dtData = null;
            List<GenderModel> mItems = new List<GenderModel>();
            GenderModel mItem = null;
            SqlDataAdapter adapter = null;
            string jsonData = string.Empty;
            try
            {
                log.DebugFormat("Retrieving GetAllGender ");
                using (SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["iCovCon"].ToString()))
                {
                    SqlParameter outErrorCode = new SqlParameter("@o_ErrorCode", SqlDbType.Int) { Direction = ParameterDirection.Output };
                    SqlParameter outErrorDesc = new SqlParameter("@o_ErrorDescription", SqlDbType.VarChar, 5000) { Direction = ParameterDirection.Output };
                    SqlCommand cmd = new SqlCommand("SP_GET_ALL_MGENDER", con);
                    ...
                    if (dtData.Rows.Count > 0)
                    {
                        mItems = new List<GenderModel>();
                        for (int i = 0; i < dtData.Rows.Count; i++)
                        {
                            var vals = dtData.Rows[i].ItemArray;
                            mItem = new GenderModel
                            {
                                id = StringUtil.getInt(vals[0].ToString()),
                                gender = StringUtil.nullIfEmpty(vals[1].ToString()),
                            };
                            mItems.Add(mItem);
                        }
                    }
                    ...
                    string errorCode = outErrorCode.Value.ToString();
                    string errorDesc = outErrorDesc.Value.ToString();
                    log.DebugFormat("GetAllGenderModel Result:{0}", errorDesc);
                    log.DebugFormat("Procedure executed successfully, GetAllGenderModel Records returned-{0}", dtData != null ? dtData.Rows.Count : 0);
                }
            }
            catch (Exception ex)
            {
        ...
        public HttpResponseMessage InsertGender(GenderModel gender)
        {
            string errorDesc = string.Empty;
            string errorCode = string.Empty;
            try
            {
                log.DebugFormat("Retrieving InsertGender ");
                ...
                using (SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["iCovCon"].ToString()))
                {
                    ...
                    SqlCommand cmd = new SqlCommand("SP_GENDER_INSERT", con);
                    cmd.CommandType = CommandType.StoredProcedure;
                    cmd.Parameters.AddWithValue("@i_id", gender.id);
                    cmd.Parameters.AddWithValue("@i_gender", gender.gender);
                    cmd.Parameters.Add(outErrorCode);
                    cmd.Parameters.Add(outErrorDesc);
                    con.Open();
                    cmd.ExecuteNonQuery();
                    errorCode = outErrorCode.Value.ToString();
                    errorDesc = outErrorDesc.Value.ToString();
                    log.DebugFormat("InsertGender Result:{0}", errorDesc);
                    log.DebugFormat("Procedure executed successfully, InsertGender Records returned-{0}", errorDesc);
                    log.Info("Insert Data:::" + JsonConvert.SerializeObject(errorDesc));
                }
            }
            catch (Exception ex)
        ...
        public HttpResponseMessage UpdateGender(GenderModel gender)
        {
            string errorDesc = string.Empty;
            string errorCode = string.Empty;
            try
            {
                log.DebugFormat("Retrieving UpdateGender ");
                ...
                using (SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["iCovCon"].ToString()))
                {
                    ...
                    SqlCommand cmd = new SqlCommand("SP_GENDER_UPDATE", con);
                    cmd.CommandType = CommandType.StoredProcedure;
                    cmd.Parameters.AddWithValue("@i_id", gender.id);
                    cmd.Parameters.AddWithValue("@i_gender", gender.gender);
                    cmd.Parameters.Add(outErrorCode);
                    ...
                    errorCode = outErrorCode.Value.ToString();
                    errorDesc = outErrorDesc.Value.ToString();
                    log.DebugFormat("UpdateGender Result:{0}", errorDesc);
                    log.DebugFormat("Procedure executed successfully, UpdateGender Records returned-{0}", errorDesc);
                    log.Info("Insert Data:::" + JsonConvert.SerializeObject(errorDesc));
                }
            }
            catch (Exception ex)
        ...
        public HttpResponseMessage InsertUsers(UserModel user)
        {
            ...
            try
            {
                ...
                using (SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["iCovCon"].ToString()))
                {
                    ...
                    cmd.Parameters.AddWithValue("@i_countryid", user.countryid);
        ...
        public HttpResponseMessage DeleteUser(int User_id)
        {
            string errorDesc = string.Empty;
            string errorCode = string.Empty;
            try
            {
                log.DebugFormat("Retrieving DeleteUser ");
                ...
                using (SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["iCovCon"].ToString()))
                {
                    ...
                    cmd.CommandType = CommandType.StoredProcedure;
                    cmd.Parameters.AddWithValue("@i_User_id", User_id);
                    cmd.Parameters.Add(outErrorCode);
                    cmd.Parameters.Add(outErrorDesc);
                    con.Open();
                    cmd.ExecuteNonQuery();
                    errorCode = outErrorCode.Value.ToString();
                    errorDesc = outErrorDesc.Value.ToString();
                    log.DebugFormat("DeleteCCUser Result:{0}", errorDesc);
                    log.DebugFormat("Procedure executed successfully, DeleteUser Records returned-{0}", errorDesc);
                    log.Info("Delete records Data:::" + User_id);
                }
            }
            catch (Exception ex)
            {
                log.ErrorFormat("Exception in DeleteUser method in database layer:{0}|{1}", ex.Message, ex.StackTrace);
            }
            return Request.CreateResponse(HttpStatusCode.OK, errorDesc, "application/json");
        }
        #endregion
        ...
        public List<PropertyImagesModel> AddImage()
        {
            ...
            try
            {
                ...
                foreach (string file in httpRequest.Files)
                {
                    ...
                    var postedFile = httpRequest.Files[file];
                    var filePath = System.IO.Path.Combine(root + "\\UploadProperty\\", postedFile.FileName);
                    postedFile.SaveAs(filePath);
                    objModel.imagepath = root + "\\UploadProperty\\" + postedFile.FileName;
                    objModel.imagename = postedFile.FileName;
